...
# JPK 12/12/2018: This program has been modified to:
#   (1) default to $STRAND_SPECIFIC = 0
#   (2) only allow hybrids if they're on the same strand
#   (3) Set $ANTISENSE_PENALTY = 0
#   (4) check if certain variables are defined before using them, specifically
#       when no gtf, geneFam, or rmsk files are specified 
# This will fix the fact that we lack directionality, but that we know the two
# sections making up a hybrid should come from the SAME strand.
...
# JK 12/12/2018: I tried changing this and it DID make a small difference
our $STRAND_SPECIFIC = 1; # transcriptome mapping.-->SETTING TO 0 IS NOT RECOMMENDED!

# JK 12/13/2018: This doesn't appear to change the outcome at all... Score is determined
# in the processAlignRec subroutine, where:
#   score = MapQ(a) + MapQ(b) + $HYBRID_PENALTY + $antiPenalty
# (where antiPenalty is based off the scores and whether the reads were reverse-
# complemented or not, multiplied by the $ANTISENSE_PENALTY).
# Larger scores = better
our $HYBRID_PENALTY = 0;#-24; # this should be optimized.
our $ANTISENSE_PENALTY = 0;#-24; #so should this  #DEPRECATED VARIABLE 2/2015

# JPK 12/12/2018: Added same strandedness requirement in case we want to enforce no
# hybrids between opposite strands
our $REQ_SAME_STRAND = 0;
...
# Added 12/13/2018 to allow manipulating how much space in between or how much overlap
# can between two alignments there can be and still consider them a hybrid. 1 is the
# default that was being used originally.
my $overlapAllowed = 1;
my $gapAllowed = 1;
...
GetOptions("o=s" => \$outputCore, 
           ...
           "noanti=i" => \$STRAND_SPECIFIC, # Updated 12/12/2018 to allow integer passed in
           "antipen=i" => \$ANTISENSE_PENALTY, # Added 12/12/2018
           "gfam=s" => \$geneFamFile,
           "gtf=s" => \$gtfFile,
           "rmsk=s" => \$rmskFile,
           "pen=i" => \$HYBRID_PENALTY,
           "ss=i" => \$REQ_SAME_STRAND, # Added 12/12/2018
           "overlap=i" => \$overlapAllowed, # Added 12/13/2018
           "gap=i" => \$gapAllowed # Added 12/13/2018
);
...
sub processAlignRec {
  ...
  foreach my $end (sort keys %$eHash) {
    ...
    # updated to allow overlap parameter to define this
    for (my $i=1; $i <= $overlapAllowed; $i++) {
      if(defined($sHash->{$end - $i})) {
        push(@$findList, @{$sHash->{$end - $i}});
        push(@$found, $end - $i);
      }
    }
    # updated to allow gap parameter to define this
    for (my $i=1; $i <= $gapAllowed; $i++) {
      if(defined($sHash->{$end + $i})) {
        push(@$findList, @{$sHash->{$end + $i}});
        push(@$found, $end + $i);
      }
    ...
    foreach my $a (@$curList) {
      foreach my $b (@$findList) {
        ...
        # JPK 12/12/2018: declared $a_strand and $b_strand as variables
        my $a_strand = $alnHash->{$a}->[4] eq "hybrid" ? $alnHash->{$a}->[6] : getStrand($alnHash->{$a}->[4]);
        my $b_strand = $alnHash->{$b}->[4] eq "hybrid" ? $alnHash->{$b}->[6] : getStrand($alnHash->{$b}->[4]);

        # JPK 12/12/2018: Added to prevent different-strand hybrids
        next unless ($REQ_SAME_STRAND == 0) or ($a_strand eq $b_strand);
        ...
        # JPK 12/12/2018: Added $a_strand to the end
        $alnHash->{"$a\:$b"} = [$score, $start, $len, $readName, "hybrid", $geneIdHyb, $a_strand];
...
sub alignCmp {
  ...
  return($cmp) if $cmp != 0;
  if(defined($biotypeA) and defined($biotypeB)) {
    $cmp = (biorank($biotypeA) <=> biorank($biotypeB)); # biotype ranking
  }
  return($cmp) if $cmp != 0;
  if(defined($symB) and defined($symA)) {
...
sub biorank {
  my $biotype = shift;
  return $biotype unless defined($biotype); # JPK 12/12/2018 Quality of Life change
...
sub isOverAbundant {
  my($name) = @_;;
  my $ret = 0;
  return $ret unless defined($name); # JPK 12/12/2018 Quality of Life change
