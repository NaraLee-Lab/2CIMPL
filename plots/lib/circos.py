...
conf_template = jpk_util.getDir('../plots/circos/template.conf')
influenza_karyotype = jpk_util.getDir('../plots/circos/influenza_karyotype.txt')
...
def makeInteractionFiles(link_file, dir='.', prefix='', chrom_size_file=wsn_chroms):
	...
	dir = jpk_util.getDir(dir)
	try:
		#inter_file = makeIntersegFile(link_file, dir=dir, prefix=prefix)
...
def runCircosFromLinks(links_file, dir='.', prefix='', chrom_size_file=wsn_chroms,
	colored=False, hitsclip_bg_path=None, include_hist=True, ribbon=False):
	...
	juncs, bg = makeInteractionFiles(links_file, dir=circos_dir,
		prefix=prefix, chrom_size_file=chrom_size_file)
