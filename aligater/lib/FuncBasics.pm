...
sub shove {
  # shifts the first value from @_ (the args), i.e. it pops the
  # first item. $aRef will be assigned to what was previously the
  # first value in @_, and @_ will now have one less member.
  # The first arg should have been an array.
  my $aRef = shift;
  my(@val) = @_;
  # If the first argument passed in was an array, then push the
  # value(s) onto the array
  if(ref($aRef) eq 'ARRAY') {
    push(@$aRef, @val);
  } else {
    # Otherwise create the array and THEN push the values onto it
    $aRef = [];
    push(@$aRef, @val);
  }
  # Return the array
